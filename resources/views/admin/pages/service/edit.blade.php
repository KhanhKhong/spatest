        ...
        <div class="col-lg-10">
            ...
            <div class="panel panel-default">
                <form action="{{ route('service.update') }}" method="post" enctype="multipart/form-data">
                     {{ csrf_field() }}
                     <input type="text" hidden="true" name="service_id" value="{{ isset($editItems) ? $editItems->id : '' }}">
                    <div class="panel-body">
                        <div class="row">
							<div class="col-lg-10">
                                ...
                                <div class="form-group">
                                    <label>price</label>
                                    <input type="number" class="form-control" name="price" value="{{ isset($editItems) ? $editItems->price : '' }}">
                                </div>
                                <div class="form-group">
                                    <label>Category</label>
									<select class="js-example-basic-single form-control" name="service_category_id">
						                <option value="0">Select Category</option>
                                        @foreach($cate as $row) 
                                            <option value="{{ $row->id }}" {{ (isset($editItems) ? $editItems->service_category_id : '') == $row->id ? 'selected="selected"' : '' }}>{{ $row->title }}</option>
                                        @endforeach
									</select>
                                </div>
							</div>
